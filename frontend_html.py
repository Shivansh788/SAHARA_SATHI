...
<head>
    ...
    <style>
        ...
        #main-sidebar.sidebar-collapsed .nav-item .material-symbols-outlined {
            margin: 0;
        }

        #main-sidebar.sidebar-collapsed .nav-item:hover::after {
            content: attr(title);
            position: absolute;
            left: 5.8rem;
            background: #222b44;
        ...
        #main-sidebar.sidebar-collapsed ~ #main-content {
            padding-left: 5.5rem;
        }

        #main-sidebar:not(.sidebar-collapsed) ~ #main-content {
            padding-left: 18rem;
        }
...
<body class="bg-background text-on-surface min-h-screen">
        ...
        <div class="relative z-10 w-full max-w-md mx-4">
            ...
            <div id="org-login-form" class="bg-white/10 backdrop-blur-2xl rounded-3xl p-8 border border-white/20 shadow-2xl" style="display:none;">
                ...
                <div class="mt-8 border-t border-white/10 pt-6">
                    <h3 class="text-xl font-bold text-white mb-4 font-['Manrope']">Register Organization</h3>
                    <form id="org-register-form" class="flex flex-col gap-4" enctype="multipart/form-data" onsubmit="handleOrgRegister(event)">
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Organization Name</label>
                            <input id="org-register-name" name="name" type="text" placeholder="Enter organization name" class="w-full h-12 px-5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']" required />
                        </div>
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Email</label>
                            <input id="org-register-email" name="email" type="email" placeholder="Enter email" class="w-full h-12 px-5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']" required />
                        </div>
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Password</label>
                            <input id="org-register-pass" name="password" type="password" placeholder="Create a password" class="w-full h-12 px-5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']" required />
                        </div>
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Category</label>
                            <input id="org-register-category" name="category" type="text" placeholder="e.g. Healthcare, Education" class="w-full h-12 px-5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']" required />
                        </div>
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Location</label>
                            <input id="org-register-location" name="location" type="text" placeholder="e.g. Jaipur, Rajasthan" class="w-full h-12 px-5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']" required />
                        </div>
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Description</label>
                            <textarea id="org-register-description" name="description" placeholder="Describe your organization" class="w-full h-20 px-5 py-2 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']"></textarea>
                        </div>
                        <div>
                            <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Upload Logo/Certificate (optional)</label>
                            <input id="org-register-file" name="file" type="file" class="w-full text-white" accept="image/*,.pdf" />
                        </div>
                        <button type="submit" class="w-full h-12 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl font-bold text-sm hover:from-orange-600 hover:to-orange-700 transition-all shadow-lg shadow-orange-500/30 flex items-center justify-center gap-2 font-['Plus_Jakarta_Sans']">
                            <span class="material-symbols-outlined text-lg">how_to_reg</span>
                            Register Organization
                        </button>
                    </form>
                    <div id="org-register-status" class="mt-4 text-sm text-orange-200"></div>
                    <form id="org-otp-form" class="flex flex-col gap-3 mt-4" style="display:none;" onsubmit="handleOrgOTP(event)">
                        <label class="text-blue-200 text-xs font-bold uppercase tracking-widest mb-2 block font-['Plus_Jakarta_Sans']">Enter OTP (sent to email)</label>
                        <input id="org-otp-code" name="otp" type="text" maxlength="6" class="w-full h-12 px-5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-blue-300/50 focus:outline-none focus:ring-2 focus:ring-orange-400/50 text-sm font-['Plus_Jakarta_Sans']" required />
                        <button type="submit" class="w-full h-12 bg-gradient-to-r from-orange-400 to-orange-500 text-white rounded-xl font-bold text-sm hover:from-orange-500 hover:to-orange-600 transition-all flex items-center justify-center gap-2 font-['Plus_Jakarta_Sans']">
                            <span class="material-symbols-outlined text-lg">verified</span>
                            Verify & Activate
                        </button>
                        <div id="org-otp-status" class="mt-2 text-sm text-orange-200"></div>
                    </form>
                </div>
            </div>
    ...
    <!-- Side Navigation (Mobile Hidden) - Sidebar Reworked -->
    <aside id="main-sidebar" class="h-screen w-72 sidebar-open fixed left-0 top-0 bg-slate-50 border-r border-outline-variant/10 pt-24 pb-8 flex flex-col justify-between z-40 transition-all duration-300 ease-in-out overflow-hidden">
        <div class="px-4 flex flex-col gap-2">
            <button id="sidebar-hamburger" class="w-10 h-10 rounded-lg bg-white border border-slate-200 text-primary flex items-center justify-center shadow-sm" onclick="toggleSidebar()" title="Toggle sidebar" aria-label="Toggle sidebar">
                <span class="material-symbols-outlined">menu</span>
            </button>
        </div>
        <div id="sidebar-items" class="mt-4 flex flex-col gap-1 px-2 flex-1">
            <a class="nav-link-side nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-blue-900 font-bold hover:bg-blue-50" onclick="switchTab('home')" data-target="home" title="Home">
                <span class="material-symbols-outlined">home</span>
                <span class="sidebar-text">Home</span>
            </a>
            <a class="nav-link-side nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-500 hover:text-blue-900 hover:bg-blue-50" onclick="switchTab('schemes')" data-target="schemes" title="Schemes">
                <span class="material-symbols-outlined">account_balance</span>
                <span class="sidebar-text">Schemes</span>
            </a>
            <a class="nav-link-side nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-500 hover:text-blue-900 hover:bg-blue-50" onclick="switchTab('applications')" data-target="applications" title="Applications">
                <span class="material-symbols-outlined">description</span>
                <span class="sidebar-text">Applications</span>
            </a>
            <a class="nav-link-side nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-500 hover:text-blue-900 hover:bg-blue-50" onclick="switchTab('chat')" data-target="chat" title="Chat Assistant">
                <span class="material-symbols-outlined">forum</span>
                <span class="sidebar-text">Chat Assistant</span>
            </a>
            <a class="nav-link-side nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-500 hover:text-blue-900 hover:bg-blue-50" onclick="switchTab('profile')" data-target="profile" title="Profile">
                <span class="material-symbols-outlined">person</span>
                <span class="sidebar-text">Profile</span>
            </a>
            <a class="nav-link-side nav-item flex items-center gap-3 px-3 py-3 mt-8 rounded-xl font-bold text-base bg-gradient-to-r from-orange-400 to-orange-500 text-white shadow-md border-2 border-orange-300 hover:from-orange-500 hover:to-orange-600 hover:scale-[1.03] transition-all focus:outline-none focus:ring-2 focus:ring-orange-300" style="box-shadow: 0 2px 12px 0 rgba(255,140,0,0.13); min-width:0; width:88%; margin-left:auto; margin-right:auto;" onclick="openAuth('org-login')" title="Volunteer an Event">
                <span class="material-symbols-outlined text-xl">volunteer_activism</span>
                <span class="sidebar-text">Volunteer an Event</span>
            </a>
        </div>
        <div class="flex items-start w-full px-4 pb-3 pt-6">
            <button onclick="openAuth('org-login')"
                class="sidebar-cta-btn flex items-center gap-2 justify-start px-6 py-3 rounded-xl font-bold text-base bg-primary text-white w-[88%] shadow-md hover:bg-primary-container transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary"
                style="min-width: 0;">
                <span class="material-symbols-outlined text-xl">volunteer_activism</span>
                <span>Volunteer an Event</span>
            </button>
        </div>
        <style>
        .sidebar-cta-btn {
            margin-left: 0;
            margin-right: auto;
            box-shadow: 0 2px 12px 0 rgba(0,0,60,0.08);
        }
            ...
            <div id="view-events" class="view-section hidden-view flex flex-col gap-8">
                ...
                <div class="bg-white rounded-3xl premium-shadow p-6 border border-slate-100">
                    <div class="grid md:grid-cols-4 gap-4">
                        ...
                        <button onclick="applyEventsFilter()" class="h-12 px-5 rounded-xl bg-primary text-white font-bold text-sm hover:bg-primary-container">Apply Filters</button>
                ...
                <div id="org-admin-panel" class="bg-white p-8 rounded-[2rem] premium-shadow border border-slate-100 hidden-view">
                    <div class="flex items-center justify-between gap-4 border-b pb-4 mb-6">
                        <div>
                            ...
                            <p id="org-admin-subtitle" class="text-sm text-slate-500 mt-1">Create and manage your events below.</p>
                        </div>
                    </div>
                    <form id="org-event-form" class="grid md:grid-cols-2 gap-4" onsubmit="handleOrgEventCreate(event)">
                        <input id="event-title" name="title" type="text" placeholder="Event title (e.g., Healthcare Camp)" class="h-12 px-4 rounded-xl border border-slate-200" required />
                        <input id="event-category" name="category" type="text" placeholder="Category (e.g., Healthcare)" class="h-12 px-4 rounded-xl border border-slate-200" required />
                        <input id="event-location" name="location" type="text" placeholder="Location (e.g., Dehradun)" class="h-12 px-4 rounded-xl border border-slate-200" required />
                        <input id="event-start" name="start_date" type="date" class="h-12 px-4 rounded-xl border border-slate-200" required />
                        <input id="event-end" name="end_date" type="date" class="h-12 px-4 rounded-xl border border-slate-200" required />
                        <div></div>
                        <textarea id="event-description" name="description" placeholder="Event details" class="md:col-span-2 min-h-28 px-4 py-3 rounded-xl border border-slate-200" required></textarea>
                        <div class="md:col-span-2 flex gap-3 mt-5">
                            <button type="submit" class="px-6 py-3 rounded-xl bg-primary text-white font-bold text-sm hover:bg-primary-container">Create Event</button>
                        </div>
                        <div id="org-event-status" class="md:col-span-2 mt-2 text-sm text-orange-500"></div>
                    </form>
                </div>
                                    // Organization Event Creation Logic
                                    async function handleOrgEventCreate(event) {
                                        event.preventDefault();
                                        const statusDiv = document.getElementById('org-event-status');
                                        statusDiv.textContent = '';
                                        const form = document.getElementById('org-event-form');
                                        ...
                                        try {
                                            ...
                                            if (data.ok) {
                                                ...
                                                statusDiv.textContent = data.message || 'Event creation failed.';
                                            }
                                        } catch (e) {
    ...
    <script>
                        ...
                        let orgRegisterOrgId = null;
                        let orgRegisterEmail = '';
                        let orgRegisterPassword = '';
                        async function handleOrgRegister(event) {
                            event.preventDefault();
                            const statusDiv = document.getElementById('org-register-status');
                            statusDiv.textContent = '';
                            document.getElementById('org-otp-form').style.display = 'none';
                            ...
                            orgRegisterEmail = payload.email;
                            orgRegisterPassword = payload.password;
                            try {
                                const resp = await fetch('/api/org/register', {
                                    method: 'POST',
                                ...
                                const data = await resp.json();
                                if (data.ok) {
                                    statusDiv.textContent = 'Registered! Please check your email for OTP.';
                                    orgRegisterOrgId = data.org_id;
                                    document.getElementById('org-otp-form').style.display = 'flex';
                                } else {
                                    statusDiv.textContent = data.message || 'Registration failed.';
                                }
                            } catch (e) {
                                statusDiv.textContent = 'Error registering organization.';
                            }
                        }

                        async function handleOrgOTP(event) {
                            event.preventDefault();
                            const otpDiv = document.getElementById('org-otp-status');
                            otpDiv.textContent = '';
                            const code = document.getElementById('org-otp-code').value.trim();
                            if (!orgRegisterOrgId) {
                                otpDiv.textContent = 'No registration in progress.';
                                return;
                            }
                            try {
                                const resp = await fetch('/api/org/verify', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ org_id: orgRegisterOrgId, otp_code: code })
                                });
                                const data = await resp.json();
                                if (data.ok) {
                                    otpDiv.textContent = 'Organization verified! Logging you in...';
                                    // Auto-login after verification
                                    await handleOrgAutoLogin();
                                } else {
                                    otpDiv.textContent = data.message || 'OTP verification failed.';
                                }
                            } catch (e) {
                                otpDiv.textContent = 'Error verifying OTP.';
                            }
                        }

                        async function handleOrgAutoLogin() {
                            // Use the email and password from registration
                            if (!orgRegisterEmail || !orgRegisterPassword) return;
                            try {
                                const resp = await fetch('/api/org/login', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ email: orgRegisterEmail, password: orgRegisterPassword })
                                });
                                const data = await resp.json();
                                if (data.ok && data.token) {
                                    // Store token, show org admin panel, close overlay
                                    localStorage.setItem('org_token', data.token);
                                    showToast('Organization onboarded!');
                                    // Optionally redirect or show org admin panel
                                    document.getElementById('auth-screen').style.display = 'none';
                                    // Show org admin panel if present
                                    const orgPanel = document.getElementById('org-admin-panel');
                                    if (orgPanel) orgPanel.classList.remove('hidden-view');
                                } else {
        ...
        function submitTranscript(transcript) {
            ...
            if (!text) {
                appendBotMessage(window.currentLanguage === 'hindi' ? 'आवाज समझ नहीं आई, कृपया दोबारा बोलें।' : window.currentLanguage === 'hinglish' ? 'Voice clear nahi mili, please dobara try karo.' : 'Could not understand voice input, please try again.');
                return;
            }
